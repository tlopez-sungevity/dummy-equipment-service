...
class SlickBackedEquipmentService @Inject()(@NamedDatabase("equipment") dbConfigProvider: DatabaseConfigProvider) extends EquipmentService {
	...
	class EquipmentTable(tag: Tag) extends Table[(Int, Int, Int, String, Option[String], DateTime, Option[Double], Option[Double], Option[Double], Option[Double], Option[Boolean], Option[Double], Option[Double], Option[Double], Option[Double], Option[Double], Option[Boolean])](tag, "equipment") {
		def id = column[Int]("id", O.PrimaryKey) // This is the primary key column
		...
		def * = (id, equipmentTypeId, manufacturerId, model, description, modifiedDate, panelKwStc, panelKwPtc, panelHeightMm, panelWidthMm, panelIsBipvRated, powerTempCoefficient, normalOperatingCellTemperature, rating, inverterEfficiency, inverterOutputVoltage, inverterIsThreePhase)
	...
	def getEquipment(equipmentId: Int): Future[Option[Equipment]] = {
		val equipmentQuery = for { 
  		...
  		foundEquipment map { results => results.headOption map { 
  			case (id, "module", modelName, manufacturerName, description, modifiedDate,
  				...
  				None, None, None, isThreePhase) => 
  					Module(id, modelName, manufacturerName, description, modifiedDate, panelKwStc, panelKwPtc, panelHeightMm, panelWidthMm, panelIsBipvRated, powerTempCoefficient, normalOperatingCellTemperature)
  			case (id, "inverter" , modelName, manufacturerName, description, modifiedDate, 
